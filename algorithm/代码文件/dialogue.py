...
from .write_result import lst_to_csv
import os
import re
import time
...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    result = []
    import pandas as pd
    data = pd.read_csv(r'webui_pages\dialogue\prompt_B.csv',encoding='utf-8')
    data1 = data.iloc[:,0]
    ...
    # prompts = ['如果信号是稀疏的 ，可由远高于采样定理要求的采样点重建恢复。',' IDMA 在每个用户传输的信号里填入了大量的零，这样做可以大大降低用户间的干扰。'
    #            ,'IDMA 系统的用户数 K=6 对应稀疏 IDMA 系统的激活用户数 大于 IDMA 系统的用户数 K=4时 对应稀疏 IDMA 系统的激活户数']
    # prompts = ['本文提出的混合五电平单相整流器利用四叉树数据结构进行有效的点云表示和压缩。\
    #            The hybrid five-level single-phase rectifier proposed in the paper utilizes a quadtree data structure for efficient point cloud representation and compression.',
    # '非地面网络（NTN）与核心网络架构的集成是实现卫星与移动设备直接连接的技术方向。The integration of non-terrestrial networks (NTN) with core network architecture is a technological\
    #       direction for enabling direct satellite connectivity with mobile devices.']
    #dialogue_mode = '知识库问答'
